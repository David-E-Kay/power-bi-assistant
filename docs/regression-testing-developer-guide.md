...
| **Per-model helpers** | `scripts/gen-{model}-testlines.py` + `scripts/apply-{model}-testlines.py` | Generate the testLines + groupByColumns blocks and inject them into a fresh copy of the capture script | Update the embedded measure list when the model changes; otherwise leave alone | Standalone Python — no dependencies beyond stdlib |

### How they fit together

```mermaid
flowchart TD
    T[scripts/capture-snapshot.csx<br/>READ-ONLY template] -->|copied by| H[apply-{model}-testlines.py]
    G[scripts/gen-{model}-testlines.py] -->|emits testLines + groupByColumns| H
    H -->|writes| O[output/{label}.csx<br/>session-specific copy]
    ...
    classDef helper fill:#cce5ff,stroke:#004085
...
Currently, helpers exist only for M&C (`gen-mc-testlines.py`, `apply-mc-testlines.py`). When new models are onboarded, follow the same naming convention. The helper pattern is optional — for one-off tests you can copy the template and inject testLines manually.
...
set MODEL_NAME=Maintenance and Construction

# Baseline capture against the original model
set SNAPSHOT_LABEL=baseline
TabularEditor.exe "C:\path\to\original-model.bim" -S "output\mc-regression.csx"
...
TabularEditor.exe "C:\path\to\refactored-model.bim" -S "output\mc-regression.csx"
...
>
> Or, if a per-model helper exists (`apply-mc-testlines.py`), it edits these sections for you.

---

## 8. First-Day Walkthrough

Scenario: you're a new developer on day one. The lead engineer asks you to validate a relationship change in the M&C model — a new direct foreign key between `Work Orders` and `Properties` is being added, replacing an older bridge-table path.

### Step 1: Plan the test (5 min, with Claude)

Open Claude in this project and say:
> "I'm validating a refactor in M&C — adding a direct FK from Work Orders to Properties to replace the existing bridge path. Help me build a regression test."
...
For M&C, the helper does this for you:

```bash
python scripts\apply-mc-testlines.py
```

This reads `scripts/capture-snapshot.csx` (the template), invokes `gen-mc-testlines.py` to produce 195 measures × 10 contexts = 1,950 test cases, and writes the result to `output/mc-baseline.csx`.

For a new model without a helper, ask Claude to draft the testLines and groupByColumns blocks for you — or copy-paste manually following the format in §7.

### Step 3: Capture baseline (~5 min at TOPN=5)

**GUI path:**
1. Open the original `.pbip` in TE3
2. Open `output/mc-baseline.csx` in TE3's script tab
...
```bash
set SNAPSHOT_LABEL=baseline
TabularEditor.exe "C:\path\to\original-mc.bim" -S "output\mc-baseline.csx"
```

### Step 4: Apply the change

Run the refactor script (or make the change manually in TE3, or deploy the modified `.bim`). For our example, that's adding the new active relationship between `Work Orders` and `Properties`.
...
```bash
set SNAPSHOT_LABEL=refactored
TabularEditor.exe "C:\path\to\modified-mc.bim" -S "output\mc-baseline.csx"
```
...
export MODEL_NAME="Maintenance and Construction"
TabularEditor.exe ./models/mc/model.bim -S ./output/mc-regression.csx
